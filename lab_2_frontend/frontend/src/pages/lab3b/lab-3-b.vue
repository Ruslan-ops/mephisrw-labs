<template>
  <div class="q-pa-md flex column g-m">
    <div class="text-h6 text-primary text-center text-bold">
      3Б - Лабораторная работа "Нечеткий логический вывод"
    </div>
    <div
    ...
    <q-tab-panels v-model="tab" animated class="shadow-2 rounded-borders">
      ...
      <q-tab-panel name="result-1">
        <result-first
          :variant="variant"
          :next_matrices_headers="nextMatricesHeaders"
        />
      </q-tab-panel>
...
localStorage.setItem('lab-token', 'dsjfnwp9u82n39f85n324079vn0we0unsidvkjnvv7');

const userId = computed(() => labStore.userId);
const variant = computed(() => labStore.variant);
const matrix = computed(() => labStore.matrix);
const nextMatricesHeaders = computed(() => labStore.nextMatricesHeaders);
