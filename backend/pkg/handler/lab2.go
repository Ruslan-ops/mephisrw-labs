...
func (h *Handler) OpenSecondLab(c *gin.Context) {
	...
	userDone, matrix, err := h.Service.GetVariance(ctx, userId, service.Lab2Id)
	if err != nil {
...
func (h *Handler) OpenSecondLabForStudent(c *gin.Context) {
	...
	if isOpenBool {
		isDone, err := h.Service.OpenLabForStudent(ctx, userId, service.Lab2Id, externalLabId)
		if err != nil {
				...
				for flag {
					...
					default:
						...
						if firstMaxA > secondMaxA+0.03 && firstMaxB > secondMaxB+0.03 && firstMaxC > secondMaxC+0.03 {
							if err := h.Service.UpdateUserVariance(ctx, userId, service.Lab2Id, variance); err != nil {
								return
							}
							return
						}
